...
df = pd.read_csv('D:/Microinjection_Project/Python_Code/Camera/Calibration_data_collection_ProjectionMatrix_all.csv')
data = df.values
...
'''
Ax = 0
A = |   0         0          0          0         dX dy1   dY dy1|
    |   0         0          0          0        -dX dx1  -dY dx1|
    |-dX dy1   -dY dy1     dX dx1     dY dx1        0        0   | (3 x 6)
    
A = |   0         0          0          0         dX dy2   dY dy2|
    |   0         0          0          0        -dX dx2  -dY dx2|
    |-dX dy2   -dY dy2     dX dx2     dY dx2        0        0   | (3 x 6)
'''
...
for i in range(len(data)):
    current_A_1_1 = np.array([0, 0, 0, 0, data[i, 5]*data[i, 2], data[i, 6]*data[i, 2]])
    current_A_1_2 = np.array([0, 0, 0, 0, -data[i, 5]*data[i, 1], -data[i, 6]*data[i, 1]])
    current_A_1_3 = np.array([-data[i, 5]*data[i, 2], -data[i, 6]*data[i, 2], data[i, 5]*data[i, 1], data[i, 6]*data[i, 1], 0, 0])
    current_A_1 = np.vstack([current_A_1_1, current_A_1_2, current_A_1_3])
    A1 = np.vstack([A1, current_A_1])
    
A1 = np.delete(A1, (0), axis=0)
A1 = np.asmatrix(A1)
P1, D1, Q1 = np.linalg.svd(A1, full_matrices=True)
# A1_try = P1 @ np.diag(D1) @ Q1
Proj_mat_1 = np.array([[Q1[5, 0], Q1[5, 1]], [Q1[5, 2], Q1[5, 3]], [Q1[5, 4], Q1[5, 5]]])
...
Pix_diff_xy = Proj_mat_1 @ np.array([[dX], [dY]])
Pix_diff = np.array([[0, 0, dy1], [0, 0, -dx1], [-dy1, dx1, 0]])
Trial_3 = Pix_diff @ Proj_mat_1 
