...
xyz_data = np.load("D:/Microinjection_Project/Python_Code/Injection_Images/03-28-2022_11-33-44/change_xyz.npy", allow_pickle=True)
yolk_data = np.load("D:/Microinjection_Project/Python_Code/Injection_Images/03-28-2022_11-33-44/yolk_data.npy", allow_pickle=True)

# Arrange data in required order to save
all_data = np.empty([1, 6])
for i in range(len(xyz_data)):
    for j in range(len(xyz_data[i])):
        current_row = np.array([yolk_data[i][j][0], yolk_data[i][j][1], yolk_data[i][j][2], yolk_data[i][j][3], xyz_data[i][j][0], xyz_data[i][j][1]])
...
pd.DataFrame(all_data).to_csv("Calibration_data_collection.csv")
